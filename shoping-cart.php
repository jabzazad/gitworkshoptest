					...
					<div class="m-l-25 m-r--38 m-lr-0-xl">
						<div class="wrap-table-shopping-cart">
							<table class="table-shopping-cart">
								...
								$query="select * from cart INNER JOIN product ON  product.Pid = cart.Pid where cart.Mid=".$_SESSION['Mid'];
								$result = mysqli_query($connect, $query);
								while($row = mysqli_fetch_array($result)){
									echo '<form action="delete-cart.php" method="post">';
								echo '<tr>';
								echo '<td class="column-2">'.$row['Pname'].'</td>';
								
								echo '<td class="column-1">'.$row['Pprice'].'</td>';
								echo '<td class="column-3">'.$row['Qproduct'].'</td>';
								echo '<td class="column-1">'.$row['Pprice']*$row['Qproduct'].'</td>';
								echo '<td>';
								echo '<input type="hidden" value='.$row['Pid'].' name="DeleteValue">';
								echo '<input type="submit" value="Cancel">';
								echo '</td>';
								$total+=$row['Pprice']*$row['Qproduct'];
								echo '</tr>';
								echo '</form>';
								}
							
							echo '</table><br>';
							echo "<h4>Total price without shipping =$".$total."</h4>";
							?>
						</div>
						...
						<div class="flex-w flex-t bor12 p-t-15 p-b-30">
							...
							<div class="size-209 p-r-18 p-r-0-sm w-full-ssm">
								...
								<div class="p-t-15">
									...
									<?php
								echo '<div class="rs1-select2 rs2-select2 bor8 bg0 m-b-12 m-t-9">
...
$result = mysqli_query($connect, $query);
while($row = mysqli_fetch_array($result)){
											echo '<option value="'.$row['PROVINCE_NAME_ENG'].'">'.$row['PROVINCE_NAME_ENG'].'</option>';
}
									...
									<div class="bor8 bg0 m-b-12">
										<input class="stext-111 cl8 plh3 size-111 p-lr-15" type="text" name="amphur" placeholder="Amphur"required>
									</div>
...
if($total==0){echo '<button type="submit" class="flex-c-m stext-101 cl0 size-116 bg3 bor14 hov-btn3 p-lr-15 trans-04 pointer" disabled>
							Proceed to Checkout
						</button>';}else{
